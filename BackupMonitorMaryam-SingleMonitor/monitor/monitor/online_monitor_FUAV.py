...
class ROSMonitor_online_monitor_FUAV(Node):
	...
	def callbackcmd_velX(self,data):
		self.get_logger().info("monitor has observed "+ str(data))
		dict= rosidl_runtime_py.message_to_ordereddict(data)
		dict['topic']='cmd_velX'
		dict['time']=float(self.get_clock().now().to_msg().sec)
	...
	def __init__(self,monitor_name,log,actions):
		...
		self.topics_info['cmd_velX']={'package': 'std_msgs.msg', 'type': 'Int16'}
		...
		self.config_subscribers['cmd_velX']=self.create_subscription(topic='cmd_velX',msg_type=Int16,callback=self.callbackcmd_velX,qos_profile=1000)
